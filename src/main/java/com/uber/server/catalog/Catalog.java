...
public class Catalog {
     ...
     * @return Pet instance
     */
    public com.uber.server.pets.Pet generatePetFromRow(Map<String, Object> row) {
        if (row == null || game == null || game.getPetRepository() == null) {
            return null;
        }
        
        return com.uber.server.pets.Pet.fromRow(row, game.getPetRepository(), game);
    }
        ...
        switch (itemType) {
            ...
            case "h":
                ...
                if (habbo.getSubscriptionManager().hasSubscription("habbo_club")) {
                    com.uber.server.users.subscriptions.Subscription sub = 
